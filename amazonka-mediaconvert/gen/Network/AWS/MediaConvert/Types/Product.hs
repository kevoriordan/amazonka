...
--
-- * 'hsWriteMp4PackagingType' - Undocumented member.
--
...
data M2tsSettings = M2tsSettings'
  { _mPmtPid               :: !(Maybe Nat)
  , _mVideoPid             :: !(Maybe Nat)
  , _mBufferModel          :: !(Maybe M2tsBufferModel)
  , _mProgramNumber        :: !(Maybe Nat)
  , _mScte35Pid            :: !(Maybe Nat)
  , _mMinEbpInterval       :: !(Maybe Nat)
  , _mTransportStreamId    :: !(Maybe Nat)
  , _mMaxPcrInterval       :: !(Maybe Nat)
  , _mFragmentTime         :: !(Maybe Double)
  , _mPrivateMetadataPid   :: !(Maybe Nat)
...
--
-- * 'mForceTsVideoEbpOrder' - Undocumented member.
--
...
m2tsSettings =
  M2tsSettings'
    ...
    , _mForceTsVideoEbpOrder = Nothing
...
mPmtPid :: Lens' M2tsSettings (Maybe Natural)
mPmtPid = lens _mPmtPid (\ s a -> s{_mPmtPid = a}) . mapping _Nat

-- | Packet Identifier (PID) of the elementary video stream in the transport stream.
mVideoPid :: Lens' M2tsSettings (Maybe Natural)
mVideoPid = lens _mVideoPid (\ s a -> s{_mVideoPid = a}) . mapping _Nat
...
mProgramNumber :: Lens' M2tsSettings (Maybe Natural)
mProgramNumber = lens _mProgramNumber (\ s a -> s{_mProgramNumber = a}) . mapping _Nat

-- | Packet Identifier (PID) of the SCTE-35 stream in the transport stream.
mScte35Pid :: Lens' M2tsSettings (Maybe Natural)
mScte35Pid = lens _mScte35Pid (\ s a -> s{_mScte35Pid = a}) . mapping _Nat

-- | When set, enforces that Encoder Boundary Points do not come within the specified time interval of each other by looking ahead at input video. If another EBP is going to come in within the specified time interval, the current EBP is not emitted, and the segment is "stretched" to the next marker. The lookahead value does not add latency to the system. The Live Event must be configured elsewhere to create sufficient latency to make the lookahead accurate.
mMinEbpInterval :: Lens' M2tsSettings (Maybe Natural)
mMinEbpInterval = lens _mMinEbpInterval (\ s a -> s{_mMinEbpInterval = a}) . mapping _Nat
...
mMaxPcrInterval :: Lens' M2tsSettings (Maybe Natural)
mMaxPcrInterval = lens _mMaxPcrInterval (\ s a -> s{_mMaxPcrInterval = a}) . mapping _Nat
...
mPrivateMetadataPid :: Lens' M2tsSettings (Maybe Natural)
mPrivateMetadataPid = lens _mPrivateMetadataPid (\ s a -> s{_mPrivateMetadataPid = a}) . mapping _Nat
...
mTimedMetadataPid :: Lens' M2tsSettings (Maybe Natural)
mTimedMetadataPid = lens _mTimedMetadataPid (\ s a -> s{_mTimedMetadataPid = a}) . mapping _Nat
...
mPcrPid :: Lens' M2tsSettings (Maybe Natural)
mPcrPid = lens _mPcrPid (\ s a -> s{_mPcrPid = a}) . mapping _Nat
...
mDvbSubPids :: Lens' M2tsSettings [Natural]
...
-- | Undocumented member.
mForceTsVideoEbpOrder :: Lens' M2tsSettings (Maybe M2tsForceTsVideoEbpOrder)
mForceTsVideoEbpOrder = lens _mForceTsVideoEbpOrder (\ s a -> s{_mForceTsVideoEbpOrder = a})
...
mAudioPids :: Lens' M2tsSettings [Natural]
mAudioPids = lens _mAudioPids (\ s a -> s{_mAudioPids = a}) . _Default . _Coerce

-- | Packet Identifier (PID) for input source DVB Teletext data to this output.
mDvbTeletextPid :: Lens' M2tsSettings (Maybe Natural)
mDvbTeletextPid = lens _mDvbTeletextPid (\ s a -> s{_mDvbTeletextPid = a}) . mapping _Nat
...
data Mp2Settings = Mp2Settings'
  { _mssChannels   :: !(Maybe Nat)
  , _mssSampleRate :: !(Maybe Nat)
...
-- | Set Channels to specify the number of channels in this output audio track. Choosing Mono in the console will give you 1 output channel; choosing Stereo will give you 2. In the API, valid values are 1 and 2.
mssChannels :: Lens' Mp2Settings (Maybe Natural)
mssChannels = lens _mssChannels (\ s a -> s{_mssChannels = a}) . mapping _Nat

-- | Sample rate in hz.
mssSampleRate :: Lens' Mp2Settings (Maybe Natural)
mssSampleRate = lens _mssSampleRate (\ s a -> s{_mssSampleRate = a}) . mapping _Nat

-- | Average bitrate in bits/second.
mssBitrate :: Lens' Mp2Settings (Maybe Natural)
