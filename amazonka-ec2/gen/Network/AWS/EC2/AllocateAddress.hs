...
data AllocateAddress = AllocateAddress'
  { _aaPublicIPvfourPool :: !(Maybe Text)
...
--
-- * 'aaPublicIPvfourPool' - The ID of an address pool that you own. Use this parameter to let Amazon EC2 select an address from the address pool. To specify a specific address from the address pool, use the @Address@ parameter instead.
--
...
aaPublicIPvfourPool :: Lens' AllocateAddress (Maybe Text)
aaPublicIPvfourPool = lens _aaPublicIPvfourPool (\ s a -> s{_aaPublicIPvfourPool = a})
...
instance AWSRequest AllocateAddress where
        ...
        response
          = receiveXML
              (\ s h x ->
                 AllocateAddressResponse' <$>
                   (x .@? "publicIpv4Pool") <*> (x .@? "allocationId")
                     <*> (x .@? "domain")
                     <*> (x .@? "publicIp")
                     <*> (pure (fromEnum s)))
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'aarsPublicIPvfourPool' - The ID of an address pool.
--
...
allocateAddressResponse pResponseStatus_ =
  AllocateAddressResponse'
    { _aarsPublicIPvfourPool = Nothing
...
aarsPublicIPvfourPool :: Lens' AllocateAddressResponse (Maybe Text)
aarsPublicIPvfourPool = lens _aarsPublicIPvfourPool (\ s a -> s{_aarsPublicIPvfourPool = a})
