...
--
-- * 'dvpcpcNextToken' - The token to request the next page of results. (You received this token from a prior call.)
--
...
--
-- * 'dvpcpcMaxResults' - The maximum number of results to return for this request. The request returns a token that you can specify in a subsequent call to get the next set of results.
describeVPCPeeringConnections
    :: DescribeVPCPeeringConnections
describeVPCPeeringConnections =
  DescribeVPCPeeringConnections'
