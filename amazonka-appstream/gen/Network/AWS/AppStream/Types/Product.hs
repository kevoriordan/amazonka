...
--
-- * 'fDisconnectTimeoutInSeconds' - The time after disconnection when a session is considered to have ended, in seconds. If a user who was disconnected reconnects within this time interval, the user is connected to their previous session. Specify a value between 60 and 360000. By default, this value is 900 seconds (15 minutes).
--
-- * 'fMaxUserDurationInSeconds' - The maximum time that a streaming session can run, in seconds. Specify a value between 600 and 360000.
--
...
--
-- * 'sAuthenticationType' - The authentication method. The user is authenticated using a streaming URL (@API@ ), SAML 2.0 federation (@SAML@ ), or the AppStream 2.0 user pool (@USERPOOL@ ). The default is to authenticate users using a streaming URL.
--
