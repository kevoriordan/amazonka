...
--
-- * 'gdcesCatalogId' - The ID of the Data Catalog for which to retrieve the security configuration. If none is supplied, the AWS account ID is used by default.
getDataCatalogEncryptionSettings
    :: GetDataCatalogEncryptionSettings
getDataCatalogEncryptionSettings =
...
instance AWSRequest GetDataCatalogEncryptionSettings
         where
...
instance ToHeaders GetDataCatalogEncryptionSettings
         where
        toHeaders
          = const
              (mconcat
...
instance ToJSON GetDataCatalogEncryptionSettings
         where
        toJSON GetDataCatalogEncryptionSettings'{..}
          = object
              (catMaybes [("CatalogId" .=) <$> _gdcesCatalogId])

instance ToPath GetDataCatalogEncryptionSettings
         where
        toPath = const "/"

instance ToQuery GetDataCatalogEncryptionSettings
         where
...
data GetDataCatalogEncryptionSettingsResponse = GetDataCatalogEncryptionSettingsResponse'
  { _gdcesrsDataCatalogEncryptionSettings :: !(Maybe DataCatalogEncryptionSettings)
  , _gdcesrsResponseStatus :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
getDataCatalogEncryptionSettingsResponse
    :: Int -- ^ 'gdcesrsResponseStatus'
    -> GetDataCatalogEncryptionSettingsResponse
getDataCatalogEncryptionSettingsResponse pResponseStatus_ =
  GetDataCatalogEncryptionSettingsResponse'
    { _gdcesrsDataCatalogEncryptionSettings = Nothing
