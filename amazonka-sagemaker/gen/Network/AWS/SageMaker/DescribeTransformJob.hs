...
--
-- * 'dtjrsFailureReason' - If the transform job failed, the reason that it failed.
--
-- * 'dtjrsBatchStrategy' - If you want to include only one record in a batch, specify @SingleRecord@ .. If you want batches to contain a maximum of the number of records specified in the @MaxPayloadInMB@ parameter, specify @MultiRecord@ .S
--
