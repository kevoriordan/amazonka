...
--
--
-- For more information about algorithms provided by Amazon SageMaker, see <http://docs.aws.amazon.com/sagemaker/latest/dg/algos.html Algorithms> . For information about using your own algorithms, see <http://docs.aws.amazon.com/sagemaker/latest/dg/your-algorithms.html Using Your Own Algorithms with Amazon SageMaker> .
--
--
...
--
-- * 'asTrainingImage' - The registry path of the Docker image that contains the training algorithm. For information about docker registry paths for built-in algorithms, see <http://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-algo-docker-registry-paths.html Algorithms Provided by Amazon SageMaker: Common Parameters> .
--
...
-- | The registry path of the Docker image that contains the training algorithm. For information about docker registry paths for built-in algorithms, see <http://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-algo-docker-registry-paths.html Algorithms Provided by Amazon SageMaker: Common Parameters> .
...
continuousParameterRange pName_ pMinValue_ pMaxValue_ =
  ContinuousParameterRange'
    {_cName = pName_, _cMinValue = pMinValue_, _cMaxValue = pMaxValue_}
...
--
-- * 'hptjdVPCConfig' - The 'VpcConfig' object that specifies the VPC that you want the training jobs that this hyperparameter tuning job launches to connect to. Control access to and from your training container by configuring the VPC. For more information, see <http://docs.aws.amazon.com/sagemaker/latest/dg/train-vpc.html Protect Training Jobs by Using an Amazon Virtual Private Cloud> .
--
-- * 'hptjdEnableInterContainerTrafficEncryption' - To encrypt all communications between ML compute instances in distributed training, specify @True@ . Encryption provides greater security for distributed training, but training take longer because of the additional communications between ML compute instances.
--
...
--
-- * 'hptjcStrategy' - Specifies the search strategy for hyperparameters. Currently, the only valid value is @Bayesian@ .
--
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'hptjwscParentHyperParameterTuningJobs' - An array of hyperparameter tuning jobs that are used as the starting point for the new hyperparameter tuning job. For more information about warm starting a hyperparameter tuning job, see <http://docs.aws.amazon.com/automatic-model-tuning-incremental Using a Previous Hyperparameter Tuning Job as a Starting Point> . Hyperparameter tuning jobs created before October 1, 2018 cannot be used as parent jobs for warm start tuning jobs.
--
...
integerParameterRange pName_ pMinValue_ pMaxValue_ =
  IntegerParameterRange'
    {_iprName = pName_, _iprMinValue = pMinValue_, _iprMaxValue = pMaxValue_}
...
--
-- * 'mdRegex' - A regular expression that searches the output of a training job and gets the value of the metric. For more information about using regular expressions to define metrics, see <http://docs.aws.amazon.com/sagemaker/latest/dg/automatic-model-tuning-define-metrics.html Defining Objective Metrics> .
metricDefinition
    :: Text -- ^ 'mdName'
    -> Text -- ^ 'mdRegex'
    -> MetricDefinition
...
-- | A regular expression that searches the output of a training job and gets the value of the metric. For more information about using regular expressions to define metrics, see <http://docs.aws.amazon.com/sagemaker/latest/dg/automatic-model-tuning-define-metrics.html Defining Objective Metrics> .
...
--
-- For information about notebook instance lifestyle configurations, see <http://docs.aws.amazon.com/sagemaker/latest/dg/notebook-lifecycle-config.html Step 2.1: (Optional) Customize a Notebook Instance> .
--
--
...
--
-- * 'nisNotebookInstanceLifecycleConfigName' - The name of a notebook instance lifecycle configuration associated with this notebook instance. For information about notebook instance lifestyle configurations, see <http://docs.aws.amazon.com/sagemaker/latest/dg/notebook-lifecycle-config.html Step 2.1: (Optional) Customize a Notebook Instance> .
--
...
-- | The name of a notebook instance lifecycle configuration associated with this notebook instance. For information about notebook instance lifestyle configurations, see <http://docs.aws.amazon.com/sagemaker/latest/dg/notebook-lifecycle-config.html Step 2.1: (Optional) Customize a Notebook Instance> .
...
--
-- * 'tjVPCConfig' - A 'VpcConfig' object that specifies the VPC that this training job has access to. For more information, see <http://docs.aws.amazon.com/sagemaker/latest/dg/train-vpc.html Protect Training Jobs by Using an Amazon Virtual Private Cloud> .
--
...
--
-- * 'tjTags' - An array of key-value pairs. For more information, see <http://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/cost-alloc-tags.html#allocation-what Using Cost Allocation Tags> in the /AWS Billing and Cost Management User Guide/ .
--
...
-- | A 'VpcConfig' object that specifies the VPC that this training job has access to. For more information, see <http://docs.aws.amazon.com/sagemaker/latest/dg/train-vpc.html Protect Training Jobs by Using an Amazon Virtual Private Cloud> .
...
--
-- * 'tjdTrainingInputMode' - The input mode used by the algorithm for the training job. For the input modes that Amazon SageMaker algorithms support, see <http://docs.aws.amazon.com/sagemaker/latest/dg/algos.html Algorithms> . If an algorithm supports the @File@ input mode, Amazon SageMaker downloads the training data from S3 to the provisioned ML storage Volume, and mounts the directory to docker volume for training container. If an algorithm supports the @Pipe@ input mode, Amazon SageMaker streams data directly from S3 to the container.
--
...
-- | The input mode used by the algorithm for the training job. For the input modes that Amazon SageMaker algorithms support, see <http://docs.aws.amazon.com/sagemaker/latest/dg/algos.html Algorithms> . If an algorithm supports the @File@ input mode, Amazon SageMaker downloads the training data from S3 to the provisioned ML storage Volume, and mounts the directory to docker volume for training container. If an algorithm supports the @Pipe@ input mode, Amazon SageMaker streams data directly from S3 to the container.
...
newtype TransformDataSource = TransformDataSource'
  { _tdsS3DataSource :: TransformS3DataSource
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
transformDataSource
    :: TransformS3DataSource -- ^ 'tdsS3DataSource'
    -> TransformDataSource
...
instance FromJSON TransformDataSource where
        parseJSON
          = withObject "TransformDataSource"
              (\ x ->
                 TransformDataSource' <$> (x .: "S3DataSource"))

instance Hashable TransformDataSource where

instance NFData TransformDataSource where
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'tiSplitType' - The method to use to split the transform job's data into smaller batches. If you don't want to split the data, specify @None@ . If you want to split records on a newline character boundary, specify @Line@ . To split records according to the RecordIO format, specify @RecordIO@ . The default value is @None@ .  Amazon SageMaker sends the maximum number of records per batch in each request up to the MaxPayloadInMB limit. For more information, see <http://mxnet.io/architecture/note_data_loading.html#data-format RecordIO data format> .
--
...
data TransformJobDefinition = TransformJobDefinition'
  { _tjdBatchStrategy           :: !(Maybe BatchStrategy)
...
transformJobDefinition
    :: TransformInput -- ^ 'tjdTransformInput'
    -> TransformOutput -- ^ 'tjdTransformOutput'
    -> TransformResources -- ^ 'tjdTransformResources'
    -> TransformJobDefinition
transformJobDefinition pTransformInput_ pTransformOutput_ pTransformResources_ =
  TransformJobDefinition'
...
instance FromJSON TransformJobDefinition where
        parseJSON
          = withObject "TransformJobDefinition"
              (\ x ->
                 TransformJobDefinition' <$>
                   (x .:? "BatchStrategy") <*> (x .:? "MaxPayloadInMB")
                     <*> (x .:? "Environment" .!= mempty)
...
instance Hashable TransformJobDefinition where

instance NFData TransformJobDefinition where
...
transformJobSummary
    :: Text -- ^ 'tjsTransformJobName'
    -> Text -- ^ 'tjsTransformJobARN'
    -> UTCTime -- ^ 'tjsCreationTime'
    -> TransformJobStatus -- ^ 'tjsTransformJobStatus'
    -> TransformJobSummary
transformJobSummary pTransformJobName_ pTransformJobARN_ pCreationTime_ pTransformJobStatus_ =
  TransformJobSummary'
...
instance FromJSON TransformJobSummary where
        parseJSON
          = withObject "TransformJobSummary"
              (\ x ->
                 TransformJobSummary' <$>
                   (x .:? "FailureReason") <*>
                     (x .:? "LastModifiedTime")
...
instance Hashable TransformJobSummary where

instance NFData TransformJobSummary where
...
data TransformOutput = TransformOutput'
  { _toAssembleWith :: !(Maybe AssemblyType)
...
--
-- * 'toS3OutputPath' - The Amazon S3 path where you want Amazon SageMaker to store the results of the transform job. For example, @s3://bucket-name/key-name-prefix@ . For every S3 object used as input for the transform job, the transformed data is stored in a corresponding subfolder in the location under the output prefix. For example, for the input data @s3://bucket-name/input-name-prefix/dataset01/data.csv@ the transformed data is stored at @s3://bucket-name/key-name-prefix/dataset01/@ . This is based on the original name, as a series of .part files (.part0001, part0002, etc.).
transformOutput
    :: Text -- ^ 'toS3OutputPath'
    -> TransformOutput
...
instance FromJSON TransformOutput where
        parseJSON
          = withObject "TransformOutput"
              (\ x ->
                 TransformOutput' <$>
                   (x .:? "AssembleWith") <*> (x .:? "Accept") <*>
                     (x .:? "KmsKeyId")
                     <*> (x .: "S3OutputPath"))

instance Hashable TransformOutput where

instance NFData TransformOutput where
...
--
-- * 'trInstanceType' - The ML compute instance type for the transform job. For using built-in algorithms to transform moderately sized datasets, ml.m4.xlarge or @ml.m5.large@ should suffice. There is no default value for @InstanceType@ .
--
-- * 'trInstanceCount' - The number of ML compute instances to use in the transform job. For distributed transform, provide a value greater than 1. The default value is @1@ .
transformResources
    :: TransformInstanceType -- ^ 'trInstanceType'
    -> Natural -- ^ 'trInstanceCount'
    -> TransformResources
...
instance FromJSON TransformResources where
        parseJSON
          = withObject "TransformResources"
              (\ x ->
                 TransformResources' <$>
                   (x .:? "VolumeKmsKeyId") <*> (x .: "InstanceType")
                     <*> (x .: "InstanceCount"))

instance Hashable TransformResources where

instance NFData TransformResources where
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'tsdsS3DataType' - If you choose @S3Prefix@ , @S3Uri@ identifies a key name prefix. Amazon SageMaker uses all objects with the specified key name prefix for batch transform.  If you choose @ManifestFile@ , @S3Uri@ identifies an object that is a manifest file containing a list of object keys that you want Amazon SageMaker to use for batch transform.
--
...
transformS3DataSource
    :: S3DataType -- ^ 'tsdsS3DataType'
    -> Text -- ^ 'tsdsS3URI'
    -> TransformS3DataSource
transformS3DataSource pS3DataType_ pS3URI_ =
  TransformS3DataSource' {_tsdsS3DataType = pS3DataType_, _tsdsS3URI = pS3URI_}
...
instance FromJSON TransformS3DataSource where
        parseJSON
          = withObject "TransformS3DataSource"
              (\ x ->
                 TransformS3DataSource' <$>
                   (x .: "S3DataType") <*> (x .: "S3Uri"))

instance Hashable TransformS3DataSource where

instance NFData TransformS3DataSource where
...
instance Hashable USD where

instance NFData USD where
...
newtype UiConfig = UiConfig'
  { _ucUiTemplateS3URI :: Text
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ucUiTemplateS3URI' - The Amazon S3 bucket location of the UI template. For more information about the contents of a UI template, see <http://docs.aws.amazon.com/sagemaker/latest/dg/sms-custom-templates-step2.html Creating Your Custom Labeling Task Template> .
uiConfig
    :: Text -- ^ 'ucUiTemplateS3URI'
    -> UiConfig
uiConfig pUiTemplateS3URI_ = UiConfig' {_ucUiTemplateS3URI = pUiTemplateS3URI_}


-- | The Amazon S3 bucket location of the UI template. For more information about the contents of a UI template, see <http://docs.aws.amazon.com/sagemaker/latest/dg/sms-custom-templates-step2.html Creating Your Custom Labeling Task Template> .
...
instance Hashable UiTemplate where

instance NFData UiTemplate where

instance ToJSON UiTemplate where
        toJSON UiTemplate'{..}
          = object (catMaybes [Just ("Content" .= _utContent)])

-- | Specifies a VPC that your training jobs and hosted models have access to. Control access to and from your training and model containers by configuring the VPC. For more information, see <http://docs.aws.amazon.com/sagemaker/latest/dg/host-vpc.html Protect Endpoints by Using an Amazon Virtual Private Cloud> and <http://docs.aws.amazon.com/sagemaker/latest/dg/train-vpc.html Protect Training Jobs by Using an Amazon Virtual Private Cloud> .
--
--
--
...
--
-- * 'wMemberDefinitions' - The Amazon Cognito user groups that make up the work team.
--
...
workteam
    :: Text -- ^ 'wWorkteamName'
    -> NonEmpty MemberDefinition -- ^ 'wMemberDefinitions'
    -> Text -- ^ 'wWorkteamARN'
    -> Text -- ^ 'wDescription'
    -> Workteam
workteam pWorkteamName_ pMemberDefinitions_ pWorkteamARN_ pDescription_ =
  Workteam'
    { _wSubDomain = Nothing
